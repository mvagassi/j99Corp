...
function Cezpdf($paper='LETTER',$orientation='portrait',$vaOption=array(),$pagenumber='1',$nPaperIncWidth=0,$nPaperIncHeight=0){
  ...
  if(!empty($vaOption)){
    $nTopMargin       = $vaOption['nOpt_MTop']/10 ;
    $nLeftMargin      = $vaOption['nOpt_MLeft']/10 ;
    $nBottomMargin    = $vaOption['nOpt_MBottom']/10 ;
    $nRightMargin     = $vaOption['nOpt_MRight']/10 ; 
  ...
  // Default Setting 
  $this->selectFont('./sc_pdf/fonts/Helvetica.afm');
...
/**************************************************************************************************************************************************************************************************/
function ezImage($image,$lTop=false,$nHeight=0,$width = 0,$resize = 'full',$pad = 5,$just = 'center',$border = ''){
  if (stristr($image,'://')){ 
    $fp = @fopen($image,"rb") ;
    while(!feof($fp)){
      $cont.= fread($fp,1024);
...
function ezTable(&$data,$cols='',$title='',$options=''){
  ...
  $defaults = array('shaded'=>0,'showLines'=>2,'shadeCol'=>array(0.9,0.9,0.9),'shadeCol2'=>array(0.7,0.7,0.7),'fontSize'=>10,'titleFontSize'=>11
